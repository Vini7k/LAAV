...
$stmt = $mysqli->prepare("  
    ...
    GROUP BY 
        reservas.id, users.name, reservas.data_emprestimo, reservas.devolucao_prevista, reservas.horario_emprestimo, reservas.horario_devolucao_emprestimo
");

$stmt->execute();

$stmt->bind_result($userName, $data_emprestimo, $devolucao_prevista, $horario_emprestimo, $horario_devolucao_emprestimo, $aparelho_modelos);
...
while ($stmt->fetch()) {
    // Formatar o horário para mostrar apenas a hora e minuto
    $horario_emprestimo_formatado = date("H:i", strtotime($horario_emprestimo));
    $horario_devolucao_formatado = date("H:i", strtotime($horario_devolucao_emprestimo));
    ...
    echo "<td>" . htmlspecialchars($horario_emprestimo_formatado, ENT_QUOTES, 'UTF-8') . "</td>"; // Exibe o horário de empréstimo formatado
    echo "<td>" . htmlspecialchars($horario_devolucao_formatado, ENT_QUOTES, 'UTF-8') . "</td>"; // Exibe a hora de devolução formatada
